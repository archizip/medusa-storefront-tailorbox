...
export function NextIntlProvider({
  ...
  useEffect(() => {
    console.log(
      "[NextIntlProvider] Initialized with locale:",
      initialLocale,
      "-> normalized:",
  ...
  useEffect(() => {
    const getCookieLocale = () => {
      ...
      const localeCookie = cookies.find((c) =>
        c.trim().startsWith("_medusa_locale=")
      )
      const rawLocale = localeCookie
    ...
    const checkLocale = () => {
      ...
      if (newLocale && newLocale !== locale) {
        console.log(
          "[NextIntlProvider] Locale changed:",
          locale,
          "->",
          newLocale
        )
        ...
        import(`../../messages/${newLocale}.json`)
          .then((mod) => {
            console.log(
              "[NextIntlProvider] Messages loaded for locale:",
              newLocale
            )
          ...
          .catch((err) => {
            console.error(
              "[NextIntlProvider] Failed to load messages for locale:",
              newLocale,
              err
            )
            // Fallback на украинский, если переводы не найдены
